...
// GetAllPermissions は全ロール・全機能の権限マトリクスを取得する
func (r *Repository) GetAllPermissions(ctx context.Context) ([]*RolePermission, error) {
	const q = `
...
func (r *Repository) GetPermissionByRoleAndFeature(ctx context.Context, roleName, featureName string) (*PermissionCheck, error) {
	const q = `
...
func (r *Repository) UpdatePermissions(ctx context.Context, operatorID uuid.UUID, inputs []UpdateInput) error {
	...
	for _, inp := range inputs {
		...
		_, err := tx.Exec(ctx, `
			INSERT INTO role_permissions (role_id, feature_id, can_view, can_create, can_edit, can_delete, scope, updated_at)
		...
		_, err = tx.Exec(ctx, `
			INSERT INTO operation_logs (operator_id, operation_type, target_type, target_id, before_value, after_value)
			VALUES ($1, 'permission_update', 'role_permission', $2, $3, $4)`,
			operatorID, targetID, beforeJSON, afterJSON,
		)
		if err != nil {
...
func (r *Repository) EmergencyAppointment(ctx context.Context, operatorID uuid.UUID, input EmergencyAppointmentInput) error {
	...
	if _, err := tx.Exec(ctx, `
		INSERT INTO operation_logs (operator_id, operation_type, target_type, target_id, before_value, after_value)
		VALUES ($1, 'emergency_appointment', 'user', $2, $3, $4)`,
		operatorID, targetID, beforeJSON, afterJSON,
	); err != nil {
		return fmt.Errorf("insert operation log: %w", err)
	}
